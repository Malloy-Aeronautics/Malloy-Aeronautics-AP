...
void NavEKF3_core::update_gps_selection(void)
{
    ...
    if (frontend->_affinity & EKF_AFFINITY_GPS) {
        ...
        // select our preferred_gps if it has a 3D fix, otherwise use
        // the primary GPS. Receiver quality is judged by the GPS
        // auto-switch (AP_GPS::update_primary) and by EKF lane scoring,
        // not here: an accuracy gate at this point defeats GPS affinity
        // and lets a single receiver fault take down every lane.
